...
def eval(model,BATCH_SIZE=16):
    ...
    x_tv, x_test, y_tv, y_test = train_test_split(X, y_binary, test_size=0.03)
    x_train,x_val,y_train,y_val = train_test_split(x_tv,y_tv,train_size=0.9)

    for layer in disModel.layers[-3:-1]:
        print(layer.name)
    ...
    disModel.compile(loss='binary_crossentropy',
              optimizer= "adam",
    ...
    loss_val,precision_val,recall_val, f1_val=disModel.evaluate(x_test,y_test,batch_size=16)
    predictions = disModel.predict(x_test)
    # print("accuracy score: ",accuracy_score(y, p))
    # =disModel.evaluate(X,y_binary)
    print('predict',precision_val)
