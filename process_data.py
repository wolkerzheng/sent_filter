...
class TDTCorpus:
    ...
    def _get_sample_file_bnd_nltk_sent(self, key):
        ...
        sentences = self.cut_sentence_new(''.join(tokens))
...
def main(load_from_raw=True):
    ...
    for key,val in corpus.text_corpus_bnds.items():
        ...
        # if zgd_flag:
        #     print(text)
        #     print(corpus.sent_boundaries[key])
        #     zgd_flag = False
        print(text[0:3])
        print(len(corpus.sent_boundaries[key]),corpus.sent_boundaries[key])
        #segmentation = texttiling.TextTiling()

        tt = ModifiedTextTilingTokenizer(w=15,k=3,demo_mode=True)
        # tt = TextTilingTokenizer(w=20,k=10,demo_mode=True)
        try:
            gap,smooth,depth,tk, norm_bnds = tt.tokenize(text)
            # predicted_bnd = corpus.convert_char_bounds_to_binary(key,norm_bnds)
        except:
            exceptionnumber += 1
            print('texttiling exception number is :',exceptionnumber)
            continue
        predicted_bnd = corpus.convert_char_bounds_to_binary(key,norm_bnds)
